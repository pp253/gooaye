  ...
  <div class="login">
    <div class="hero">
      ...
      <div class="ticker-tape" aria-hidden="true">
        <div class="ticker-track">
          <span
            v-for="(t, i) in [...TICKER_SYMBOLS, ...TICKER_SYMBOLS]"
            :key="i"
            :class="['ticker-item', i % 2 === 0 ? 'bull' : 'bear']"
          >
            <span class="arrow">{{ i % 2 === 0 ? '▲' : '▼' }}</span
            >{{ t }}
          </span>
        </div>
      </div>
        ...
        <template v-else>
          ...
          <button class="btn google" :disabled="loggingIn" @click="google">
            <svg width="18" height="18" viewBox="0 0 48 48">
              ...
              <path
                fill="#4285F4"
                d="M46.1 24.6c0-1.6-.1-3.1-.4-4.6H24v9h12.4c-.5 2.9-2.1 5.3-4.6 7l7.1 5.5c4.2-3.9 6.6-9.6 6.6-16.9z"
              />
              <path
                fill="#FBBC05"
                d="M10.3 28.3c-.5-1.4-.7-2.9-.7-4.3s.3-3 .7-4.3l-7.8-6C.9 16.9 0 20.3 0 24s.9 7.1 2.5 10.3l7.8-6z"
              />
              <path
                fill="#34A853"
                d="M24 48c6.5 0 11.9-2.1 15.9-5.8l-7.1-5.5c-2 1.3-4.5 2.1-8.8 2.1-6.6 0-11.8-4.2-13.7-9.8l-7.8 6C6.4 42.6 14.6 48 24 48z"
              />
            </svg>
    ...
    <section class="features">
      ...
      <p class="features-sub">
        追蹤《股癌》Podcast
        主持人謝孟恭每集提到的股票，把他的觀點變成可以檢驗、可以回測的訊號——而不是聽過就忘的閒聊。
      </p>
      ...
      <p class="features-footnote">
        所有資訊僅供決策參考，不構成投資建議。過去績效不代表未來表現。
      </p>
    </section>
  </div>
</template>
...
.card {
  ...
  animation:
    card-in 0.55s cubic-bezier(0.16, 1, 0.3, 1) both,
    glow-pulse 4.5s ease-in-out 0.6s infinite;
}
...
@keyframes glow-pulse {
  ...
  50% {
    box-shadow:
      ...
      0 20px 70px -16px rgba(43, 108, 176, 0.45);
  }
}
